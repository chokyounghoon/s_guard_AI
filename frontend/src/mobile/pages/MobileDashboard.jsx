...
export default function DashboardPage({ onAiClick }) {
  ...
  return (
    <div className="min-h-screen bg-[#070b14] text-white font-sans overflow-x-clip relative">
      {/* Top Navigation */}
      <nav className="flex justify-between items-center px-4 py-3 sticky top-0 z-30"
        style={{
      ...
      >
        <div className="flex items-center space-x-3">
          ...
          >
            ...
            <span className="text-base font-black tracking-widest uppercase">
              S-GUARD <span className="text-blue-400">AI</span>
            </span>
          </div>
          ...
          <div className="flex items-center gap-1.5 px-2 py-1 rounded-full"
            ...
            <span className="text-[9px] font-black tracking-widest text-emerald-400 uppercase">Live</span>
          </div>
          ...
          <div className="flex items-center space-x-1.5">
            ...
            >
              <Cpu size={15} className="text-[#06b6d4]" />
            </button>
            ...
            >
              <BellDot size={15} className="text-[#ef4444]" />
            </button>
            ...
            <button
              ...
              className="w-8 h-8 rounded-xl flex items-center justify-center transition-all duration-300 active:scale-90"
              style={{
                background: showThresholdSettings ? 'rgba(59,130,246,0.2)' : 'rgba(255,255,255,0.05)',
                border: showThresholdSettings ? '1px solid rgba(59,130,246,0.4)' : '1px solid rgba(255,255,255,0.08)'
              }}
            >
              <Settings className={`w-3.5 h-3.5 transition-all duration-500 ${showThresholdSettings ? 'text-blue-400 rotate-45' : 'text-slate-400'} ${isSavingThreshold ? 'animate-spin' : ''}`} />
            </button>
            ...
            {(userProfile?.is_admin === 1 || userProfile?.role === 'admin') && (
              ...
              >
                <Phone size={14} className="text-[#fb923c]" />
              </button>
            )}
          </div>
        </div>
        ...
        <div className="flex items-center space-x-2">
          <button
            ...
            onPointerUp={handleTooltipEnd}
            onPointerLeave={handleTooltipEnd}
            className="w-9 h-9 rounded-2xl flex items-center justify-center transition-all duration-300 active:scale-90"
            style={{ background: 'linear-gradient(135deg, rgba(168,85,247,0.2) 0%, rgba(168,85,247,0.05) 100%)', border: '1px solid rgba(168,85,247,0.35)', boxShadow: '0 0 12px rgba(168,85,247,0.15)' }}
          >
            <Bot size={18} className="text-[#a855f7]" />
          </button>
          <div
            className="flex items-center gap-2 cursor-pointer p-1 px-2 rounded-xl transition-colors active:bg-white/5"
            onClick={() => setShowProfileModal(true)}
          >
            <div className="w-8 h-8 rounded-full flex items-center justify-center border border-white/10 overflow-hidden ring-2 ring-blue-500/20"
        ...
        {/* Floating Tooltip */}
        {activeTooltip && (
          <div className="fixed top-16 left-1/2 -translate-x-1/2 z-[200] animate-in fade-in zoom-in duration-200">
            <div className="bg-blue-600 px-3 py-1.5 rounded-xl shadow-2xl border border-blue-400/30 flex items-center gap-2">
      ...
      {messages.length > 0 && (
        <div className="fixed top-16 left-1/2 -translate-x-1/2 z-[100] w-full max-w-md p-4 space-y-2">
          {messages.map(msg => (
            <div key={msg.id} className={`flex items-center justify-between p-3 rounded-xl shadow-lg animate-in fade-in slide-in-from-top-2 duration-300 ${msg.type === 'error' ? 'bg-red-600 text-white' : 'bg-blue-600 text-white'}`}>
              <p className="text-sm font-medium">{msg.text}</p>
              <button onClick={() => dismissMessage(msg.id)} className="ml-2 p-1 rounded-full hover:bg-white/20 transition-colors shrink-0">
      ...
      {showNotifications && (
        <div className="fixed inset-0 z-[110] flex justify-end animate-in fade-in duration-300">
          ...
          <div className="w-full max-w-sm h-full shadow-2xl relative z-10 animate-in slide-in-from-right duration-500 flex flex-col border-l border-white/10"
            style={{ background: 'rgba(15,20,33,0.98)', backdropFilter: 'blur(20px)' }}>
            <div className="p-5 border-b border-white/5 flex items-center justify-between">
              ...
              <button onClick={() => setShowNotifications(false)} className="p-2 rounded-full hover:bg-white/5 transition-colors">
        ...
        <div className="rounded-3xl overflow-hidden"
          ...
          <div className="px-5 py-4 flex items-center justify-between" style={{ borderBottom: '1px solid rgba(255,255,255,0.05)' }}>
            <div className="flex items-center gap-3">
              <div className="w-9 h-9 rounded-2xl flex items-center justify-center shrink-0"
                style={{ background: 'linear-gradient(135deg, rgba(59,130,246,0.3) 0%, rgba(99,102,241,0.15) 100%)', border: '1px solid rgba(99,102,241,0.3)' }}>
                <MessageSquare className="w-4 h-4 text-blue-400" />
              </div>
              <div>
                <h2 className="font-black text-white text-sm tracking-tight">실시간 SMS 수신내역</h2>
                <p className="text-[9px] text-slate-500 font-mono uppercase tracking-widest">Incident Feed</p>
              </div>
            </div>
            <div className="flex items-center gap-2">
              {/* Hide Done Toggle */}
              <div onClick={(e) => { e.stopPropagation(); setHideCompletedSms(!hideCompletedSms); }}
                className="flex items-center gap-1.5 cursor-pointer select-none">
                <span className={`text-[9px] font-black uppercase tracking-widest ${hideCompletedSms ? 'text-blue-400' : 'text-slate-600'}`}>Done 숨김</span>
                <div className={`w-8 h-4 rounded-full p-0.5 transition-all duration-300 ${hideCompletedSms ? 'bg-blue-600' : 'bg-slate-700'}`}>
                  <div className={`w-3 h-3 bg-white rounded-full transition-all duration-300 shadow ${hideCompletedSms ? 'translate-x-4' : 'translate-x-0'}`} />
                </div>
              </div>
              {/* LIVE badge */}
              {(() => {
                ...
                return (
                  <div className={`flex items-center gap-1.5 px-2 py-1 rounded-full border transition-all duration-500 ${hasUnanalyzed ? 'bg-blue-500/10 border-blue-500/30' : 'bg-white/[0.03] border-white/10 opacity-40'}`}>
                    <span className="relative flex h-1.5 w-1.5">
                      {hasUnanalyzed && <span className="animate-ping absolute inline-flex h-full w-full rounded-full bg-blue-400 opacity-75" />}
                      <span className={`relative inline-flex rounded-full h-1.5 w-1.5 ${hasUnanalyzed ? 'bg-blue-500' : 'bg-slate-600'}`} />
                    </span>
                    <span className={`text-[9px] font-black tracking-widest ${hasUnanalyzed ? 'text-blue-400' : 'text-slate-500'}`}>{hasUnanalyzed ? 'LIVE' : 'DONE'}</span>
                  </div>
                );
              })()}
            </div>
          ...
          <div className={`transition-all duration-500 ease-in-out ${showThresholdSettings ? 'max-h-64 opacity-100 p-5' : 'max-h-0 opacity-0 overflow-hidden py-0'}`}
            style={{ borderBottom: showThresholdSettings ? '1px solid rgba(255,255,255,0.05)' : 'none', background: 'rgba(59,130,246,0.04)' }}>
            <div className="grid grid-cols-1 gap-5">
              <div className="space-y-3">
                <div className="flex justify-between items-center">
                  <div className="flex items-center gap-2">
                    <Zap className="w-3.5 h-3.5 text-blue-400" />
                    <span className="text-xs font-bold text-white uppercase tracking-wider">Technical Threshold</span>
                  </div>
              ...
              <div className="space-y-3">
                <div className="flex justify-between items-center">
                  <div className="flex items-center gap-2">
                    <MessageSquare className="w-3.5 h-3.5 text-purple-400" />
                    <span className="text-xs font-bold text-white uppercase tracking-wider">Casual Strictness</span>
                  </div>
          ...
          {/* SMS List */}
          <div className="p-3 space-y-2 overflow-y-auto max-h-[480px] scrollbar-thin">
            {visibleSms.length > 0 ? (
              visibleSms.map((msg) => {
                const isSelected = selectedSms?.inc_id === msg.inc_id;
                const isCritical = msg.severity === 'CRITICAL';
                const isMajor = msg.severity === 'MAJOR';
                const severityColor = isCritical ? 'rgba(239,68,68,0.7)' : isMajor ? 'rgba(249,115,22,0.7)' : 'rgba(99,102,241,0.5)';
                const severityBg = isCritical ? 'rgba(239,68,68,0.08)' : isMajor ? 'rgba(249,115,22,0.06)' : 'rgba(99,102,241,0.05)';
                return (
                  <div key={`sms-${msg.inc_id}`}
                    onClick={() => {
                      const isSel = selectedSms?.inc_id === msg.inc_id;
                      if (isSel) { setSelectedSms(null); setShowAgentPanel(false); setAgentMessages([]); }
                      else { setSelectedSms(msg); setShowAgentPanel(true); setAgentMessages([{ role: 'Security', text: '🔍 AI 분석을 시작합니다...', delay: 0 }]); }
                    }}
                    className="rounded-2xl p-3 flex flex-col cursor-pointer transition-all duration-200 active:scale-[0.98]"
                    style={{
                      background: isSelected ? 'rgba(234,179,8,0.05)' : severityBg,
                      border: `1px solid ${isSelected ? 'rgba(234,179,8,0.5)' : severityColor}`,
                      boxShadow: isSelected ? '0 0 20px rgba(234,179,8,0.08)' : 'none'
                    }}>
                    {/* Top row: icon + title + badges */}
                    <div className="flex items-center justify-between gap-2 mb-2">
                      <div className="flex items-center gap-2 min-w-0">
                        <div className="w-6 h-6 rounded-full flex items-center justify-center shrink-0"
                          style={{ background: isSelected ? 'rgba(234,179,8,0.15)' : `${severityColor}20` }}>
                          {msg.keyword_detected
                            ? <AlertCircle className="w-3.5 h-3.5 text-yellow-300" />
                            : <Info className="w-3.5 h-3.5" style={{ color: isSelected ? '#facc15' : '#818cf8' }} />
                          }
                        </div>
                        <h4 className="font-black text-sm truncate tracking-tight" style={{ color: isSelected ? '#facc15' : '#f1f5f9' }}>
                      ...
                      <div className="flex items-center gap-1 shrink-0">
                        {/* Severity badge */}
                        {msg.severity && (
                          <span className="h-5 flex items-center px-1.5 rounded-md text-[8px] font-black uppercase tracking-wider"
                            style={{
                              background: isCritical ? 'rgba(239,68,68,0.15)' : isMajor ? 'rgba(249,115,22,0.15)' : 'rgba(99,102,241,0.15)',
                              color: isCritical ? '#f87171' : isMajor ? '#fb923c' : '#a5b4fc',
                              border: `1px solid ${isCritical ? 'rgba(239,68,68,0.3)' : isMajor ? 'rgba(249,115,22,0.3)' : 'rgba(99,102,241,0.3)'}`
                            }}>
                            {msg.severity}
                          </span>
                        )}
                        <button onClick={(e) => { e.stopPropagation(); navigate(`/workflow/${msg.inc_id}`); }}
                          className="h-5 flex items-center gap-1 px-1.5 rounded-md text-[8px] font-black text-blue-400"
                          style={{ background: 'rgba(59,130,246,0.1)', border: '1px solid rgba(59,130,246,0.2)' }}>
                          진행상태 <ExternalLink className="w-2 h-2" />
                        </button>
                        <span className={`h-5 flex items-center px-1.5 rounded-md border text-[8px] font-black whitespace-nowrap transition-all ${
                    ...
                    <div className="flex items-center gap-2 mb-1.5">
                      <p className="text-[9px] text-slate-500 font-bold">발신: <span className="text-slate-400 font-mono">{msg.sender}</span></p>
                      {msg.employee_id && (
                        <span className="h-4 flex items-center gap-1 px-1.5 rounded text-[8px] text-blue-400 font-mono font-black"
                    ...
                    {/* Message body */}
                    <p className={`text-[12.5px] leading-relaxed break-all whitespace-pre-wrap ${isSelected ? 'text-yellow-100' : 'text-slate-300'}`}>{msg.message}</p>
                    {/* Footer */}
                    <div className="flex items-center justify-between mt-2 pt-1.5" style={{ borderTop: '1px solid rgba(255,255,255,0.04)' }}>
                      {(msg.similarity_score !== undefined && msg.similarity_score !== null) && (
                        <div className={`flex items-center gap-1 px-1.5 py-0.5 rounded text-[8px] font-black uppercase ${msg.similarity_score >= 0.8 ? 'bg-emerald-500/10 text-emerald-400 border border-emerald-500/20' : 'bg-blue-500/10 text-blue-400 border border-blue-500/20'}`}>
                          <Zap className="w-2 h-2" />Match {(msg.similarity_score * 100).toFixed(1)}%
                        </div>
                      )}
                      <span className="text-[8px] text-slate-600 font-mono ml-auto">{formatYYMMDD(msg.timestamp)}</span>
                    </div>
                  </div>
                );
              })
            ) : (
              <div className="flex flex-col items-center justify-center py-12 gap-3 opacity-25">
                <div className="w-12 h-12 rounded-2xl flex items-center justify-center" style={{ background: 'rgba(59,130,246,0.08)', border: '1px solid rgba(59,130,246,0.1)' }}>
                  <MessageSquare className="w-5 h-5 text-blue-400" />
                </div>
                <div className="text-center">
                  <p className="text-xs font-black text-slate-400 uppercase tracking-wider">수신된 SMS 없음</p>
                  <p className="text-[10px] text-slate-600 mt-0.5">장애 SMS가 수신되면 여기에 표시됩니다</p>
                </div>
              </div>
            )}
          </div>
        ...
        {/* ── BENTO CARD 2: AI Autopilot Insight (SMS 있을 때만) ── */}
        {visibleSms.length > 0 && (
          <div className="rounded-3xl overflow-hidden"
            style={{
        ...
        {/* ── BENTO CARD 3: S-Autopilot Expert Advisor ── */}
        <div className="rounded-3xl overflow-hidden"
          style={{
            background: selectedSms ? 'rgba(234,179,8,0.03)' : 'rgba(255,255,255,0.02)',
            backdropFilter: 'blur(20px)',
            WebkitBackdropFilter: 'blur(20px)',
            border: `1px solid ${selectedSms ? 'rgba(234,179,8,0.2)' : 'rgba(255,255,255,0.06)'}`,
            boxShadow: selectedSms ? '0 8px 32px rgba(234,179,8,0.06)' : '0 8px 32px rgba(0,0,0,0.25)',
          ...
          <div className="px-5 py-4 flex items-center justify-between" style={{ borderBottom: '1px solid rgba(255,255,255,0.05)' }}>
            <div className="flex items-center gap-3">
              <div className="w-9 h-9 rounded-2xl flex items-center justify-center shrink-0"
                style={{ background: 'linear-gradient(135deg, rgba(99,102,241,0.3) 0%, rgba(168,85,247,0.15) 100%)', border: '1px solid rgba(168,85,247,0.3)' }}>
                <Sparkles className="w-4 h-4 text-indigo-300" />
              </div>
              <div>
                <h2 className="font-black text-white text-sm tracking-tight">S-Autopilot Expert Advisor</h2>
                <p className="text-[9px] text-slate-500 font-mono uppercase tracking-widest">Real-time AI Response Engine</p>
              </div>
            </div>
            <div className="flex items-center gap-2">
              {/* Tab switcher */}
              <div className="flex rounded-xl overflow-hidden" style={{ border: '1px solid rgba(255,255,255,0.08)' }}>
                <button onClick={() => setActiveLogTab('ai')}
                  className={`px-3 py-1.5 text-[9px] font-black uppercase tracking-wider transition-all ${activeLogTab === 'ai' ? 'bg-indigo-600 text-white' : 'text-slate-500'}`}>
                  AI
                </button>
                <button onClick={() => setActiveLogTab('human')}
                  className={`px-3 py-1.5 text-[9px] font-black uppercase tracking-wider transition-all ${activeLogTab === 'human' ? 'bg-blue-600 text-white' : 'text-slate-500'}`}>
                  Chat
                </button>
              </div>
              {/* LIVE/DONE badge */}
              {(() => {
                ...
                return (
                  <div className={`flex items-center gap-1.5 px-2 py-1 rounded-full border transition-all duration-500 ${isDone ? 'bg-white/[0.03] border-white/10 opacity-40' : showAgentPanel && agentMessages.length > 0 ? 'bg-emerald-500/10 border-emerald-500/20' : 'bg-white/5 border-white/10'}`}>
                    <span className="relative flex h-1.5 w-1.5">
                      {!isDone && showAgentPanel && agentMessages.length > 0 && <span className="animate-ping absolute inline-flex h-full w-full rounded-full bg-emerald-400 opacity-75" />}
                      <span className={`relative inline-flex rounded-full h-1.5 w-1.5 ${isDone ? 'bg-slate-600' : showAgentPanel && agentMessages.length > 0 ? 'bg-emerald-500' : 'bg-slate-600'}`} />
                    </span>
                    <span className={`text-[9px] font-black uppercase tracking-widest ${isDone ? 'text-slate-500' : showAgentPanel && agentMessages.length > 0 ? 'text-emerald-400' : 'text-slate-600'}`}>
          ...
          {/* Panel Content - speech bubble style */}
          <div className="min-h-[400px] overflow-hidden flex flex-col">
            {showAgentPanel || selectedSms ? (
              ...
              <div className="flex flex-col items-center justify-center h-64 gap-3 opacity-20">
                ...
                <div className="text-center">
                  <p className="text-xs font-black text-slate-400 uppercase tracking-wider">인시던트를 선택하세요</p>
                  <p className="text-[10px] text-slate-600 mt-0.5">위 SMS 피드에서 항목을 탭하면 AI 분석이 시작됩니다</p>
                </div>
              </div>
            )}
          </div>
        ...
        {/* ── BENTO CARD 4: 장애 처리 현황 (Activity Flow) ── */}
        <div className="rounded-3xl overflow-hidden"
          style={{
            background: 'rgba(255,255,255,0.02)',
            backdropFilter: 'blur(20px)',
          ...
          <div className="px-5 py-4 flex items-center justify-between" style={{ borderBottom: '1px solid rgba(255,255,255,0.05)' }}>
            <div className="flex items-center gap-3">
              <div className="w-9 h-9 rounded-2xl flex items-center justify-center shrink-0"
                style={{ background: 'linear-gradient(135deg, rgba(168,85,247,0.25) 0%, rgba(99,102,241,0.12) 100%)', border: '1px solid rgba(168,85,247,0.25)' }}>
                <Activity className="w-4 h-4 text-purple-400" />
              </div>
              <div>
                <h2 className="font-black text-white text-sm tracking-tight">장애 처리 현황</h2>
                <p className="text-[9px] text-slate-500 font-mono uppercase tracking-widest">Incident MTTR Flow</p>
              </div>
            </div>
            {/* MTTR Summary (when incident selected) */}
            {selectedIncidentIdFlow && (() => {
              const startStep = incidentWorkflowSteps.find(s => s.id === 'SMS');
              const endStep = incidentWorkflowSteps.find(s => s.id === 'KNOWLEDGE');
              const startTime = startStep ? new Date(startStep.timestamp) : null;
              ...
              return (
                <div className="flex items-center gap-2">
                  ...
                  <span className={`text-sm font-black font-mono tabular-nums ${isClosed ? 'text-emerald-400' : 'text-blue-400'}`}>{formatDuration(durationMs)}</span>
                  <span className="text-[9px] font-black text-slate-500 uppercase">MTTR</span>
                </div>
              );
            })()}
          </div>
          ...
          {selectedIncidentIdFlow && (() => {
            ...
            return (
              <div className="px-5 py-3 flex items-center gap-1.5 flex-wrap" style={{ borderBottom: '1px solid rgba(255,255,255,0.04)' }}>
                {[
                  ...
                  const timeStr = calcDiff(from, to) || '-';
                  return (
                    <div key={label} className={`flex items-center gap-1 px-2 py-0.5 rounded-lg border text-[9px] font-black ${isDone ? 'bg-emerald-500/10 border-emerald-500/20 text-emerald-400' : isActive ? 'bg-blue-500/10 border-blue-500/30 text-blue-400' : 'bg-white/3 border-white/5 text-slate-600'}`}>
          ...
          <div className="p-5">
            <div className="relative">
              <div className="absolute left-[11px] top-4 bottom-4 w-[2px] bg-gradient-to-b from-blue-600/50 via-purple-500/30 to-transparent" />
              <div className="space-y-0">
                {selectedIncidentIdFlow ? (
                    ...
                    {(() => {
                      ...
                      return FLOW_STEPS.map((step, sIdx) => {
                        ...
                        if (step.id === 'RAG_AGENT') {
                          ...
                          if (rag && agent) stepData = { ...agent, id: 'RAG_AGENT', timestamp: agent.timestamp > rag.timestamp ? agent.timestamp : rag.timestamp, detail: 'AI 에이전트 그룹이 수천 건의 과거 데이터와 내부 지식베이스를 결합하여 인시던트 근본 원인을 입체적으로 분석하고 대응 시나리오를 수립했습니다.' };
                          else if (rag || agent) stepData = { ...(rag || agent), id: 'RAG_AGENT' };
                        }
                        ...
                        if (isCompleted && sIdx < FLOW_STEPS.length - 1) {
                          ...
                          if (!nextStepData && nextId === 'RAG_AGENT') nextStepData = incidentWorkflowSteps.find(s => s.id === 'RAG') || incidentWorkflowSteps.find(s => s.id === 'AGENT');
                          if (nextStepData) {
                        ...
                        const dynamicPb = intervalMinutes === 0 ? 32 : Math.min(200, Math.max(32, Math.round(32 + intervalMinutes * 0.25)));
                        return (
                          <div key={step.id} className="relative pl-14 group" style={{ paddingBottom: `${dynamicPb}px` }}>
                            {sIdx < FLOW_STEPS.length - 1 && (
                              <div className={`absolute left-[11px] top-7 bottom-[-24px] w-[2px] transition-colors duration-500 ${isCompleted ? 'bg-blue-600' : 'bg-white/5'}`} />
                            )}
                            <div className={`absolute left-0 top-0 w-6 h-6 rounded-full border-2 z-10 flex items-center justify-center transition-all duration-700 ${isCompleted ? 'bg-blue-600 border-blue-400 shadow-[0_0_20px_rgba(37,99,235,0.5)]' : isNextStep ? 'bg-blue-500/20 border-blue-400 shadow-[0_0_15px_rgba(37,99,235,0.2)]' : 'bg-gray-800 border-white/5'}`}
                              style={{ borderColor: isCompleted ? '#60a5fa' : undefined }}>
                              {isCompleted ? <CheckCircle2 className="w-3.5 h-3.5 text-white animate-in zoom-in duration-300" /> : isNextStep ? (
                            ...
                            <div className={`transition-all duration-700 ${isCompleted ? 'opacity-100' : isNextStep ? 'opacity-100 translate-x-1' : 'opacity-30'}`}>
                              <div className="flex items-center gap-2 mb-1.5 flex-wrap">
                                <h4 className={`font-black tracking-tight text-sm ${isCompleted ? 'text-white' : isNextStep ? 'text-blue-400' : 'text-gray-500'}`}>{step.label}</h4>
                                {isNextStep && (
                                  <span className="text-[9px] font-black uppercase tracking-widest px-2 py-0.5 rounded-full bg-blue-500 text-white shadow-[0_0_10px_rgba(37,99,235,0.4)]">In Progress</span>
                                )}
                                {isCompleted && <span className="text-[10px] text-white font-black font-mono bg-white/10 px-2 py-0.5 rounded">{formatYYMMDD(stepData.timestamp)}</span>}
                              </div>
                              <p className={`text-xs leading-relaxed ${isCompleted ? 'text-slate-400' : isNextStep ? 'text-slate-300 font-medium' : 'text-slate-600'}`}>
                                {isCompleted ? stepData.detail : isNextStep ? '실시간 데이터 분석 및 대응 절차를 진행 중입니다...' : '업무 단계 대기 중'}
                              </p>
                              {intervalText && sIdx < FLOW_STEPS.length - 1 && (
                                <div className="mt-2">
                                  <span className={`inline-flex items-center gap-1 text-[9px] font-bold px-2 py-0.5 rounded-full border ${intervalMinutes > 60 ? 'text-orange-400 bg-orange-500/10 border-orange-500/20' : intervalMinutes > 10 ? 'text-yellow-400 bg-yellow-500/10 border-yellow-500/20' : 'text-emerald-400 bg-emerald-500/10 border-emerald-500/20'}`}>{intervalText}</span>
                                </div>
                              )}
                              {(isCompleted || isNextStep) && step.id === 'WARROOM' && (() => {
                                ...
                                return roomExists ? (
                                  <button onClick={() => navigate(`/chat/${selectedIncidentIdFlow}`)}
                                    className="mt-2 min-h-[44px] inline-flex items-center gap-1.5 text-[11px] font-black text-white border border-blue-500/30 px-4 py-2.5 rounded-2xl bg-blue-600 shadow-[0_0_12px_rgba(37,99,235,0.3)] active:scale-95 transition-all">
                                    <Zap className="w-3.5 h-3.5" />워룸이동 <ChevronRight className="w-3 h-3" />
                                  </button>
                                ) : (
                                  <button disabled className="mt-2 min-h-[44px] inline-flex items-center gap-1.5 text-[11px] font-black text-slate-500 border border-white/5 px-4 py-2.5 rounded-2xl bg-white/5 cursor-not-allowed">
                                    <Users className="w-3.5 h-3.5 opacity-50" />워룸 미개설 (이동 불가)
                                  </button>
                                );
                              })()}
                            </div>
                  ...
                  <div className="flex flex-col items-center justify-center py-16 opacity-20">
                    <Activity className="w-12 h-12 mb-3 text-purple-400 animate-pulse" />
                    <h3 className="text-sm font-black tracking-tight text-white mb-1">인시던트 대응 모니터링 대기 중</h3>
                    <p className="text-[10px] text-slate-500 text-center leading-relaxed">SMS 피드에서 인시던트를 선택하면<br/>실시간 MTTR 흐름이 활성화됩니다</p>
                  </div>
                )}
              </div>
            </div>
