...
const SC = {
  ...
  NORMAL:   { color: '#10b981', glow: 'rgba(16,185,129,0.2)',   bg: 'rgba(16,185,129,0.04)',  border: 'rgba(16,185,129,0.18)' },
};

/* ── 등급별 원형 게이지 ── */
function SeverityOrb({ level, count, maxCount, label, isLight }) {
  const cfg = SC[level];
    ...
    <div className="flex flex-col items-center gap-2" style={{ flex: 1, minWidth: 0 }}>
      <div className="relative flex items-center justify-center" style={{ width: size, height: size }}>
        ...
        <svg width={size} height={size} className="-rotate-90" style={{ position: 'absolute', transition: 'all 0.8s ease' }}>
          <circle cx={size/2} cy={size/2} r={r} fill="none" stroke={isLight ? '#e2e8f0' : 'rgba(255,255,255,0.05)'} strokeWidth={5} />
        ...
        <div className="absolute flex flex-col items-center justify-center">
          <span className={`font-black leading-none ${isLight ? 'text-slate-900' : 'text-white'}`} style={{ fontSize: size > 70 ? 20 : 14 }}>{count}</span>
          <span className="font-black uppercase tracking-widest" style={{ fontSize: 7, color: cfg.color, opacity: 0.8 }}>건</span>
        </div>
      </div>
      <div style={{ display: 'flex', alignItems: 'center', gap: 5, padding: '4px 8px', borderRadius: 99, background: isLight ? '#f1f5f9' : cfg.bg, border: `1px solid ${isLight ? '#cbd5e1' : cfg.border}` }}>
...
function SliderRow({ icon: Icon, label, value, min, max, step, unit, isSim, color, onChange, desc, isLight }) {
  const pct = ((value - min) / (max - min)) * 100;
  return (
    <div className="space-y-2">
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-2">
          <Icon className="w-3.5 h-3.5" style={{ color: isLight ? '#64748b' : 'rgba(148,163,184,0.7)' }} />
          <span style={{ fontSize: 10, fontWeight: 700, color: isLight ? '#475569' : '#94a3b8', textTransform: 'uppercase', letterSpacing: '0.08em' }}>{label}</span>
        </div>
        <div style={{ padding: '4px 10px', borderRadius: 8, background: `${color}14`, border: `1px solid ${color}30` }}>
          ...
          <span style={{ fontSize: 9, color: isLight ? '#64748b' : '#64748b', marginLeft: 2 }}>{unit}</span>
        </div>
      </div>
      <div style={{ position: 'relative', height: 20, display: 'flex', alignItems: 'center' }}>
        <div style={{ position: 'absolute', left: 0, right: 0, height: 6, borderRadius: 99, background: isLight ? '#e2e8f0' : 'rgba(255,255,255,0.05)' }} />
        ...
        <div style={{
          ...
          background: color, border: isLight ? '3px solid #ffffff' : '3px solid #020917',
          boxShadow: `0 0 12px ${color}`, transition: 'left 0.15s',
          pointerEvents: 'none'
        }} />
      </div>
      <div style={{ display: 'flex', justifyContent: 'space-between' }}>
        <span style={{ fontSize: 8, fontFamily: 'monospace', color: isLight ? '#94a3b8' : '#334155', fontWeight: 700 }}>{isSim ? min.toFixed(2) : min}{unit}</span>
        <span style={{ fontSize: 8, fontFamily: 'monospace', color: isLight ? '#94a3b8' : '#334155', fontWeight: 700 }}>{isSim ? max.toFixed(2) : max}{unit}</span>
      </div>
    </div>
  );
}

function ThresholdBlock({ tier, title, subtitle, icon: Icon, color, bg, border, values, sliders, onChange, isLight }) {
  return (
    <div style={{ background: bg, border: isLight ? '1px solid #e2e8f0' : `1px solid ${border}`, borderRadius: 24, padding: '20px 16px', boxShadow: isLight ? '0 2px 10px rgba(0,0,0,0.03)' : 'none' }}>
      <div style={{ display: 'flex', alignItems: 'center', gap: 12, marginBottom: 20 }}>
        ...
        <div>
          ...
          <div style={{ fontSize: 9, color: isLight ? '#64748b' : '#475569', fontWeight: 600, marginTop: 1 }}>{subtitle}</div>
        </div>
      </div>
      <div style={{ display: 'flex', flexDirection: 'column', gap: 20 }}>
        {sliders.map(s => (
          <SliderRow key={s.id} {...s} color={color} value={values[s.id]} onChange={val => onChange(s.id, val)} isLight={isLight} />
        ))}
      </div>
    </div>
  );
...
export default function MobileAlertMonitor() {
  ...
  return (
    <div style={{ height: '100%', background: isLight ? '#f1f5f9' : '#020917', fontFamily: "Pretendard, sans-serif", display: 'flex', flexDirection: 'column' }} className={isLight ? 'text-slate-800' : 'text-slate-300'}>

      <header style={{ background: isLight ? 'rgba(255,255,255,0.95)' : 'rgba(2,9,23,0.9)', backdropFilter: 'blur(20px)', borderBottom: isLight ? '1px solid #e2e8f0' : '1px solid rgba(255,255,255,0.05)' }}
        className="sticky top-0 z-50 flex items-center justify-between px-4 py-3 flex-shrink-0 shadow-sm">
        <button onClick={() => goBack()}
          style={{ width: 36, height: 36, borderRadius: 12, background: isLight ? '#f1f5f9' : 'rgba(255,255,255,0.05)', border: isLight ? '1px solid #cbd5e1' : '1px solid rgba(255,255,255,0.09)', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
          <ArrowLeft style={{ width: 16, height: 16, color: isLight ? '#475569' : '#64748b' }} />
        </button>
        <div style={{ textAlign: 'center' }}>
          <div style={{ fontSize: 13, fontWeight: 900, letterSpacing: '0.18em', textTransform: 'uppercase', color: isLight ? '#0284c7' : '#06b6d4' }}>Orbital Monitor</div>
          <div style={{ fontSize: 8, fontWeight: 900, letterSpacing: '0.3em', color: isLight ? '#0284c7' : 'rgba(6,182,212,0.4)', marginTop: 2 }}>Mobile v1.0</div>
        </div>
        <button onClick={fetchIncidents}
          style={{ width: 36, height: 36, borderRadius: 12, background: isLight ? '#e0f2fe' : 'rgba(6,182,212,0.1)', border: isLight ? '1px solid #7dd3fc' : '1px solid rgba(6,182,212,0.2)', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
          <Compass style={{ width: 16, height: 16, color: isLight ? '#0284c7' : '#06b6d4', animation: isRefreshing ? 'spin 1s linear infinite' : 'none' }} />
        </button>
      </header>
      ...
      <div style={{ flex: 1, overflowY: 'auto', padding: '16px', display: 'flex', flexDirection: 'column', gap: 16, paddingBottom: 100 }}>
        ...
        {/* ── STATUS HUD ── */}
        <div style={{
          background: isLight ? '#ffffff' : `radial-gradient(ellipse at 50% -20%, ${SC[overallSev].glow} 0%, transparent 65%), rgba(255,255,255,0.02)`,
          border: isLight ? '1px solid #e2e8f0' : `1px solid ${SC[overallSev].border}`,
          borderRadius: 28,
          padding: '24px 16px',
          ...
          <div style={{ display: 'flex', justifyContent: 'center', marginBottom: 20 }}>
            <div style={{ display: 'flex', alignItems: 'center', gap: 8, padding: '8px 16px', borderRadius: 99, background: isLight ? '#f1f5f9' : SC[overallSev].bg, border: `1px solid ${isLight ? '#cbd5e1' : SC[overallSev].border}`, boxShadow: isLight ? '0 2px 8px rgba(0,0,0,0.05)' : `0 0 20px ${SC[overallSev].glow}` }}>
          ...
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', gap: 8, marginBottom: 24 }}>
            <SeverityOrb level="CRITICAL" count={critList.length}  maxCount={maxCount} label="CRITICAL" isLight={isLight} />
            <SeverityOrb level="MAJOR"    count={majorList.length} maxCount={maxCount} label="MAJOR" isLight={isLight} />
            <SeverityOrb level="NORMAL"   count={normalList.length} maxCount={maxCount} label="NORMAL" isLight={isLight} />
          </div>

          <div style={{ display: 'flex', justifyContent: 'space-between', borderTop: isLight ? '1px solid #f1f5f9' : '1px solid rgba(255,255,255,0.05)', paddingTop: 20 }}>
            {[
              { label: '전체', value: totalCount, unit: '건', color: isLight ? '#334155' : '#94a3b8' },
              ...
              <div key={item.label} style={{ textAlign: 'center' }}>
                ...
                <div style={{ fontSize: 9, fontWeight: 800, color: isLight ? '#64748b' : '#475569', textTransform: 'uppercase', marginTop: 4 }}>{item.label}</div>
              </div>
            ))}
          </div>
        </div>
        ...
        <ThresholdBlock
          ...
          bg={isLight ? '#fff1f2' : "rgba(248,113,113,0.03)"} border={isLight ? '#fecdd3' : "rgba(248,113,113,0.2)"}
        ...
        <ThresholdBlock
          ...
          bg={isLight ? '#fff7ed' : "rgba(251,146,60,0.03)"} border={isLight ? '#fed7aa' : "rgba(251,146,60,0.18)"}
        ...
        {/* LOGIC INFO */}
        <div style={{ background: isLight ? '#ffffff' : 'rgba(255,255,255,0.02)', border: isLight ? '1px solid #e2e8f0' : '1px solid rgba(255,255,255,0.06)', borderRadius: 24, padding: '16px', display: 'flex', gap: 12, boxShadow: isLight ? '0 2px 10px rgba(0,0,0,0.03)' : 'none' }}>
          <Shield style={{ width: 16, height: 16, color: isLight ? '#64748b' : '#334155', flexShrink: 0, marginTop: 2 }} />
          <div>
            <p style={{ fontSize: 10, fontWeight: 900, color: isLight ? '#334155' : '#475569', textTransform: 'uppercase', marginBottom: 8 }}>우선순위 로직</p>
            <div style={{ display: 'flex', flexDirection: 'column', gap: 6 }}>
              {[
                ...
                <div key={level} style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
                  ...
                  <span style={{ fontSize: 10, color: isLight ? '#64748b' : '#475569' }}>{rule}</span>
                </div>
              ))}
            </div>
          </div>
