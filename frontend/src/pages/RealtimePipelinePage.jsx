...
export default function RealtimePipelinePage() {
  ...
  const renderTimeline = (card) => {
    ...
    const steps = [
      { id: 'SMS', label: '인지', done: stage >= 2, active: stage === 1, dObj: null },
      { id: 'RAG', label: '분석', done: stage >= 3, active: stage === 2, dObj: diffObj(t1, t2) },
      { id: 'WARROOM', label: '워룸', done: stage >= 4, active: stage === 3, dObj: diffObj(t2, t3) },
      { id: 'KNOWLEDGE', label: '완료', done: stage >= 4, active: stage >= 4, dObj: diffObj(t3, t4) }
    ];
  ...
  const renderOperatorMode = () => {
    const trafficMap = {};
    for (let i = 0; i < 24; i++) {
      trafficMap[i.toString().padStart(2, '0') + ':00'] = 0;
    }
    const bizSystemMap = {};
    const keywordMap = {};
    const nodeMap = {};
    ...
    return (
      <div className="flex-1 flex flex-col h-full bg-[#07090f] min-h-0 overflow-hidden">
        ...
        <div className="flex-1 flex gap-4 p-4 min-h-0 overflow-hidden relative z-0">
          ...
          <div className="flex-1 flex flex-col gap-4 min-h-0 min-w-[500px]">
            {/* Top Row: Charts */}
            <div className="h-[180px] flex gap-3 shrink-0">
