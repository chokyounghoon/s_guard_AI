...
import { 
  ...
  Bell, Users, Brain, AlertCircle, Info, MessageSquare, Rocket, Heart, FileText, Trophy
} from 'lucide-react';
import { 
...
export default function RealtimePipelinePage() {
  ...
  return (
    <div className={`h-screen w-screen overflow-hidden ${isLight ? 'bg-[#F1F5F9] text-slate-800' : 'bg-zinc-950 text-slate-300'} font-sans flex flex-col pb-[60px] select-none`}>
      {!isLight && (
        <>
      ...
      {/* 🚀 Header */}
      <header className={`flex-shrink-0 flex items-center justify-between px-6 py-3 border-b ${isLight ? 'bg-white/95 border-[#E2E8F0] text-slate-900' : 'border-white/5 bg-zinc-950/90 text-white'} backdrop-blur-md z-[200]`}>
        <div className="flex items-center gap-3">
          <button onClick={() => goBack()} className={`w-9 h-9 rounded-2xl border transition-all flex items-center justify-center cursor-pointer active:scale-95 ${isLight ? 'bg-slate-100 border-slate-200 hover:bg-slate-200' : 'bg-white/5 border-white/10 hover:bg-white/10'}`}><ChevronLeft size={18} className={isLight ? 'text-slate-600' : 'text-slate-400'} /></button>
          <div>
            <h1 className={`text-sm lg:text-base font-black tracking-tight flex items-center gap-2 ${isLight ? 'text-slate-900' : 'text-white'}`}><Layers className="w-4 h-4 text-blue-600 animate-pulse" /> 통합 데시보드</h1>
            <p className="text-[9px] text-slate-500 font-bold uppercase tracking-[0.2em] mt-0.5">Realtime Pipeline & Executive Dashboard</p>
          </div>
          <button 
            onClick={() => navigate('/expert-board')} 
            className="ml-6 px-4 py-2.5 rounded-2xl bg-gradient-to-r from-amber-600/30 to-orange-600/30 hover:from-amber-500/50 hover:to-orange-500/50 border border-amber-400/40 hover:border-amber-300/70 text-amber-500 text-sm font-black shadow-[0_0_15px_rgba(245,158,11,0.2)] hover:shadow-[0_0_25px_rgba(245,158,11,0.4)] transition-all flex items-center gap-2 cursor-pointer"
          >
        ...
        <div className={`hidden md:flex items-center rounded-full p-1 border ${isLight ? 'bg-slate-100 border-slate-200' : 'bg-[#0f1422] border-white/10'}`}>
          <button onClick={() => setViewMode('operator')} className={`px-4 py-1.5 rounded-full text-[11px] font-black transition-all cursor-pointer ${viewMode === 'operator' ? (isLight ? 'bg-white text-blue-600 shadow-sm' : 'bg-[#00e5ff]/20 text-[#00e5ff] shadow-[0_0_15px_rgba(0,229,255,0.2)]') : 'text-slate-500 hover:text-slate-800'}`}>관제 모드 (조직도 및 지표 시각화)</button>
          <button onClick={() => setViewMode('executive')} className={`px-4 py-1.5 rounded-full text-[11px] font-black transition-all cursor-pointer flex items-center gap-1.5 ${viewMode === 'executive' ? (isLight ? 'bg-white text-purple-600 shadow-sm' : 'bg-purple-500/20 text-purple-400 shadow-[0_0_15px_rgba(168,85,247,0.2)]') : 'text-slate-500 hover:text-slate-800'}`}><TrendingUp className="w-3 h-3" /> 경영진 보고 모드 (D1 요약)</button>
        </div>

        <div className="flex items-center gap-2.5">
          <div className={`flex items-center border rounded-xl px-2 py-1 gap-1.5 ${isLight ? 'bg-slate-100 border-slate-200' : 'bg-[#0f1422] border-white/5'}`}>
